...
class SeatTestViewEvent_11 implements ActionListener,ItemListener{
	SeatTestView_11 stv11 = null;
	public SeatTestViewEvent_11(SeatTestView_11 stv11) {
		this.stv11 = stv11;
	}
	...
	public void itemStateChanged(ItemEvent ie) {
		...
		if (obj.getClass() == JCheckBox.class) {
			if (ie.getStateChange() == ie.SELECTED) {
				if (stv11.inwon < Integer.parseInt(stv11.jcbb_max.getSelectedItem().toString())) {
					stv11.jlb_selseatnum.setText(String.valueOf(++stv11.inwon));
					if (stv11.inwon == Integer.parseInt(stv11.jcbb_max.getSelectedItem().toString())) {
						for (JCheckBox jcb : stv11.jcb_seat) {
			...
			if (ie.getStateChange() == ie.SELECTED) {
				...
				else if(stv11.inwon > Integer.parseInt(stv11.jcbb_max.getSelectedItem().toString())){
					
					JOptionPane.showMessageDialog(stv11, "ERROR!");
				}
			}
		}
	}
...
public class SeatTestView_11 extends JFrame{
	//선언부
	//선택한 좌석수
	int inwon = 0;
	//좌석 최대 선택가능수
    ...
    SeatTestViewEvent_11 stvEvent = new SeatTestViewEvent_11(this);

    //생성자
    public SeatTestView_11() {
    	initDisplay();
    }
...
//    		jcb_seat[i].addItemListener(stvEvent);
    	...
    	for(JCheckBox jcb : jcb_seat) {
    		jcb.addItemListener(stvEvent);
