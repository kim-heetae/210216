...
class Gamemain{//게임이 진행되는 클래스
	...
	void inputN(int rnum, int num, int chance, int range) {//사용자가 생각하는 정답을 입력하는 메소드
	...
	int search(int rnum, int num, int chance, int i) {//사용자의 입력값이 정답인지 오답인지 확인하는 메소드
...
class GameSetting{//초기 값들을 정해주는 클래스 (랜덤수의 범위, 답을 입력할 수 있는 횟수)
	...
	int rdnum(int rnum, int range) {//랜덤번호를 정하기위한 메소드
		Random r = new Random();
		rnum = r.nextInt(range) + 1;
		return rnum;
	}
}
public class RandomGamePrivate1_3 {//메인 클래스
	...
	public static void main(String[] args) {
		...
		chance = gs.chanceN(chance);//기회를 정할 메소드
		rnum = gs.rdnum(rnum, range);//랜덤수를 생성
		gm.inputN(rnum, num, chance, range);//정답 입력
	}
}
